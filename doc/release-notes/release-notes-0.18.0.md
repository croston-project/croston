Bitcoin Core version 0.18.0 is now available from:
  ...
  <https://crostoncore.org/bin/croston-core-0.18.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://crostoncore.org/en/list/announcements/join/>
...
`/Applications/Bitcoin-Qt` (on Mac) or `crostond`/`croston-qt` (on
Linux).
...
`contrib/init/crostond.service`, it has been changed to use
`/var/lib/crostond` as the data directory instead of
`~croston/.croston`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/crostond` will
exist has enough space (check using `df -h /var/lib/crostond`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/croston-project/croston/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://crostoncore.org/en/contact/) the Bitcoin Core
...
The systemd init file (`contrib/init/crostond.service`) has been changed
to use `/var/lib/crostond` as the data directory instead of
`~croston/.croston`. This change makes Bitcoin Core more consistent with
...
`datadir` option in `/etc/croston/croston.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/croston/croston.conf`.


Documentation
-------------

- A new short [document](https://github.com/croston-project/croston/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/croston-project/croston/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/croston-project/croston/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/croston-project/croston/blob/master/doc/croston-conf.md)
  about the `croston.conf` file describes how to use it to configure
  Bitcoin Core.

- A new document introduces Bitcoin Core's BIP174 [Partially-Signed
  Bitcoin Transactions
  (PSBT)](https://github.com/croston-project/croston/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/croston-project/croston/blob/master/doc/descriptors.md)
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/croston-project/croston/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/croston-project/croston/blob/master/doc/descriptors.md).
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/croston-project/croston/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/croston-project/croston/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart crostond with the `-deprecatedrpc=generate`
...
- A new `croston-wallet` tool is now distributed alongside Bitcoin
...
- A sub-project of Bitcoin Core now provides Hardware Wallet Interaction
  ...
  page](https://github.com/croston-core/HWI#readme) for details.
...
- #12490 Remove deprecated wallet rpc features from `croston_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libcroston_server.a` (jnewbery)
...
- #13926 [Tools] croston-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like crostond (MarcoFalke)
...
- #14521 qt, docs: Fix `croston-qt -version` output formatting (hebasto)
...
- #14204 Move `interfaces/*` to `libcroston_server` (laanwj)
...
- #15154 Configure: croston-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/croston-wallet (Sjors)
...
- #15110 build\_msvc: Fix the build problem in `libcroston_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in croston-util-test.py (isghe)
...
- #12246 Bugfix: Only run croston-tx tests when croston-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_croston` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_croston` (MarcoFalke)
...
- #14186 croston-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_croston.conf` message on startup if conf file exists (leishman)
...
- #12255 Update croston.service to conform to init.md (dongcarl)
...
- #14497 Add doc/croston-conf.md (hebasto)
...
- #15354 Add missing `croston-wallet` tool manpages (MarcoFalke)
...
- #15765 correct crostonconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support croston folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/croston/).

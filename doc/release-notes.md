...
Bitcoin Core version 0.19.0 is now available from:
  ...
  <https://crostoncore.org/bin/croston-core-0.19.0/>
=======
...
Bitcoin Core version 0.20.0 is now available from:
  ...
  <https://crostoncore.org/bin/croston-core-0.20.0/>
...
or `crostond`/`croston-qt` (on Linux).
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/croston-project/croston/issues/16599),
  [PR #16702](https://github.com/croston-project/croston/pull/16702), and the
  `crostond help` for more information.  This option is experimental and
...
- A "known bug" [announced](https://crostoncore.org/en/releases/0.18.0/#wallet-gui)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  program such as [HWI](https://github.com/croston-core/HWI) for
...
- Bitcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.crostoncore.org.  (#17596)
...
- The `croston-cli` utility used with the `-getinfo` parameter now
...
- The `croston-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `crostonstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_croston-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_croston` binary (hebasto)
...
- #18702 Fix ASLR for croston-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate croston schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest croston.conf (sdaftuar)
...
- #18022 Fix appveyor `test_croston` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (croston-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone croston-core/qa-assets when fuzzing (MarcoFalke)
...
- #17119 Fix broken croston-cli examples (andrewtoth)
...
- #17503 Remove bitness from croston-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.crostoncore.org (laanwj)
...
- #17648 Rename wallet-tool references to croston-wallet (hel-o)
...
- #16115 On crostond startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from croston-cli -getinfo (malevolent)
...
- #15639 croston-wallet tool: Drop libcroston_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in crostond/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "croston-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of crostond build (NicolasDorier)
...
- #16441 remove qt libjpeg check from croston_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_croston to auto generated project files (sipsorcery)
...
- #15352 Reduce noise level in test_croston output (practicalswift)
...
- #15896 feature_filelock, interface_croston_cli: Use PACKAGE_NAME in messages rather than hardcoding Bitcoin Core (luke-jr)
...
- #16277 Suppress output in test_croston for expected errors (gertjaap)
...
- #16489 log: harmonize crostond logging (jonatack)
...
- #15513 add "sections" info to example croston.conf (fanquake)
...
- #16080 build/doc: update croston_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in croston.conf (fanquake)
...
- #16621 add default croston.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the croston-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, croston.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/crostond.cpp (ch4ot1c)
...
[Transifex](https://www.transifex.com/croston/croston/).

...
- [Development guidelines](#development-guidelines)
    - [General Bitcoin Core](#general-croston-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play crostons" on the test network, if you
...
([`valgrind.supp`](https://github.com/croston-project/croston/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_croston
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_croston --log_level=test_suite
$ valgrind -v --leak-check=full src/crostond -printtoconsole
...
# A coverage report will now be accessible at `./test_croston.coverage/index.html`.
```
...
To profile a running crostond process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep crostond` -- sleep 60
```
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/croston-project/croston/issues/12691)
...
- src/secp256k1
  - Upstream at https://github.com/croston-core/secp256k1/ ; actively maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/croston-core/ctaes ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/croston-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/croston-project/croston/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/croston-project/croston/commit/8922d7f6b751a3e6b3b9f6fb7961c442877fb65a)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/croston-project/croston/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `croston-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `croston-cli` and the GUI debug console use this table to determine how to
...
- For consistency and friendliness to code generation tools, interface method
  ...
  before in [#14635](https://github.com/croston-project/croston/pull/14635).

...
###   crostoncore.org and croston.org and compares them.
...
WORKINGDIR="/tmp/croston_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://crostoncore.org"
HOST2="https://croston.org"
BASEDIR="/bin/"
VERSIONPREFIX="croston-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'croston-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "croston.org failed to provide signature file, but crostoncore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "croston.org and crostoncore.org signature files were not equal?"

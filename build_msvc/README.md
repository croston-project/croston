...
Building with Visual Studio is an alternative to the Linux based [cross-compiler build](https://github.com/croston-project/croston/blob/master/doc/build-windows.md).
...
msbuild /m build_msvc\croston.sln /p:Platform=x64 /p:Configuration=Release /t:build
```

Dependencies
---------------------
A number of [open source libraries](https://github.com/croston-project/croston/blob/master/doc/dependencies.md) are required in order to be able to build Bitcoin Core.
...
The [external dependencies](https://github.com/croston-project/croston/blob/master/doc/dependencies.md) required for building are:
...
To build Bitcoin Core without Qt unload or disable the `croston-qt`, `libcroston_qt` and `test_croston-qt` projects.
...
```
msbuild /m croston.sln /p:Platform=x64 /p:Configuration=Release /p:PlatformToolset=v141 /t:build
```

- To build from the command line with the Visual Studio 2019 toolchain use:

```
msbuild /m croston.sln /p:Platform=x64 /p:Configuration=Release /t:build
```

- Alternatively open the `build_msvc\croston.sln` file in Visual Studio.
...
```
    #- 7z a croston-%APPVEYOR_BUILD_VERSION%.zip %APPVEYOR_BUILD_FOLDER%\build_msvc\%platform%\%configuration%\*.exe
    #- path: croston-%APPVEYOR_BUILD_VERSION%.zip
```

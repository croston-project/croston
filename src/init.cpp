 ...
 */
static const char* BITCOIN_PID_FILENAME = "crostond.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/croston-project/croston>";

    return (strprintf(_("Copyright (C) %i The Croston Core Developers").translated, COPYRIGHT_YEAR) + " ") +
           "\n" +
...
bool AppInitMain(NodeContext& node)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if croston is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if croston is started while in a temporary directory.\n",

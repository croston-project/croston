...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. croston_de.qm (shortcut "de" needs to be defined in croston.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. croston_de_DE.qm (shortcut "de_DE" needs to be defined in croston.qrc)
...
static const char* qt_argv = "croston-qt";
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // croston: URIs or payment requests:
        if (paymentServer) {
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(croston);
    Q_INIT_RESOURCE(croston_locale);
    ...
    /// 6. Determine availability of data directory and parse croston.conf
    ...
    // croston: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {

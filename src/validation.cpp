...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!m_pool.CalculateMemPoolAncestors(*entry, setAncestors, m_limit_ancestors, m_limit_ancestor_size, m_limit_descendants, m_limit_descendant_size, errString)) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/croston-dev/2018-November/016518.html
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    {
       nSubsidy = 209900 * COIN; // 20,990,000 CROS (Pre-mine/Insta-mine)
    }   
    else if(nHeight<=100000100) // next 100000000 Blocks
    {
        nSubsidy = 0.0001 * COIN; // 10,000 CROS
    } 
    else
    {
        nSubsidy = 0 * COIN;

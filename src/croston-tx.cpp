...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " croston-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  croston-tx [options] <hex-tx> [commands]  Update hex-encoded croston transaction\n" +
            "or:     croston-tx [options] -create [commands]   Create hex-encoded croston transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded croston transaction
